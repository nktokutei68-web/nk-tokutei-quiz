        ...
        {history.length > 0 && (
          <div className="mt-8">
            <button
              ...
              className="w-full flex items-center justify-between p-3.5 rounded-xl bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-all active:scale-[0.99]"
            >
              <div className="flex items-center gap-2.5">
                <span className="text-lg">📊</span>
                <span className="font-semibold text-slate-700 text-sm sm:text-base">Lịch sử làm bài ({history.length})</span>
              </div>
            ...
            {showHistory && (
              <div className="mt-2 space-y-2 animate-fadeIn">
                {history.map((entry) => {
                  ...
                  return (
                    <div key={entry.id} className="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                      <button
                        onClick={() => setExpandedEntry(isExpanded ? null : entry.id)}
                        className="w-full flex items-center justify-between p-3 text-left hover:bg-slate-50 transition-all active:scale-[0.99]"
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${
                            pct >= 80 ? 'bg-emerald-50 text-emerald-600' : pct >= 50 ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-500'
                          }`}>
                            {pct}%
                          </div>
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-slate-700 truncate">{getConfigLabel(entry.quizConfig)}</div>
                            <div className="text-xs text-slate-400">{formatDate(entry.date)} • {entry.score}/{entry.total} đúng</div>
                          </div>
                        </div>
                        {entry.wrongWords.length > 0 && (
                          <svg className={`w-4 h-4 text-slate-300 shrink-0 ml-2 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                          </svg>
                        )}
                      </button>
                      {isExpanded && entry.wrongWords.length > 0 && (
                        <div className="px-3 pb-3 border-t border-slate-100">
                          <p className="text-xs text-slate-500 py-2">Từ sai ({entry.wrongWords.length}):</p>
                          <div className="flex flex-wrap gap-1.5">
                            {entry.wrongWords.map((w, i) => (
                              <span key={i} className="inline-flex items-center gap-1 text-xs bg-red-50 text-red-600 px-2 py-1 rounded-md border border-red-100">
                                <span className="font-bold">{w.jp}</span>
                                <span className="text-red-400">= {w.vi}</span>
                              </span>
                            ))}
                          </div>
                        </div>
                ...
                <button
                  ...
                  className="w-full py-2.5 text-sm text-red-400 hover:text-red-500 font-medium transition-all active:scale-[0.98]"
                >
                  🗑️ Xóa lịch sử
                </button>
              </div>
